...
*/
            ...
            long long int TAT2_s = calcTimeDiff(E2, S1).tv_sec; //E2.tv_sec - S2.tv_sec;
            long int TAT2_ns = calcTimeDiff(E2, S1).tv_nsec; //E2.tv_nsec - S2.tv_nsec;
            ...
            TAT2 = ((E2.tv_sec * BIL + E2.tv_nsec) - (S1.tv_sec * BIL + S1.tv_nsec));
            totTAT = TAT1 >= TAT2 ? TAT1 : TAT2;
            ...
            if(argc == 7) {
                ...
                fprintf(fp, "%lld, %lld, %lld, %lld, %lld, %lld, %lld, %d, %d, %d\n", atoll(argv[1]), atoll(argv[2]), atoll(argv[3]), totTAT, wait12, run12, switch_ovhd, num_r_P1, num_r_P2, num_cs);
                fclose(fp);
            }
                ...
                fprintf(fp, "%lld, %lld, %lld, %lld, %lld, %lld, %lld, %lld, %lld, %d, %d, %d\n", atoll(argv[1]), atoll(argv[2]), atoll(argv[3]), atoll(argv[7]), atoll(argv[8]), totTAT, wait12, run12, switch_ovhd, num_r_P1, num_r_P2, num_cs);
                fclose(fp);
            }
            // int wait_time1_s = ;
            // int wait_time1_ns = ;
